...
export default function AddEditModal({
  ...
  async function submit(closeModal: any) {
    ...
    if (item.codigo) {
      fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/PerguntaFrequente/` + item.codigo,
        {
          method: "PUT",
        ...
        if (res.ok) {
          toast("PerguntaFrequente atualizada com sucesso.", {
            type: "success",
            autoClose: 2000,
          });
          refresh();
      ...
      fetch(`${process.env.NEXT_PUBLIC_API_URL}/PerguntaFrequente`, {
        ...
        if (res.ok) {
          toast("PerguntaFrequente criada com sucesso.", {
            type: "success",
            autoClose: 2000,
          });
          refresh();
          ...
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1">
                {item.codigo ? "Editar" : "Nova"} Pergunta Frequente
              </ModalHeader>
              <ModalBody>
                <Textarea
                  disableAutosize
