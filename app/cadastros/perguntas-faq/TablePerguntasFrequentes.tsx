...
export default function TablePerguntasFrequentes({
  items,
  refresh,
  openEditModal,
  openAddModal,
  ...
  async function deletePerguntaFrequente(codigo: string) {
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/PerguntaFrequente/${codigo}`, {
      method: "DELETE",
      headers: {
      ...
      if (res.ok) {
        toast("Pergunta frequente apagada com sucesso.", {
          type: "success",
          autoClose: 2000,
        });
        refresh();
  ...
  return (
    <>
      ...
      >
        ...
        >
          {(item: any) => (
            <TableRow key={item.codigo} className="w-screen">
              <TableCell>
              ...
              <TableCell>
                <div className="flex flex-col w-96">
                  <p className="text-bold text-sm capitalize truncate">
                    {format(new Date(item.dtInclusao), "dd/MM/yyyy")}
                  </p>
                </div>
              </TableCell>
              <TableCell>
                <div className="relative flex items-center gap-2">
                  ...
                  <Tooltip color="danger" content="Deletar">
                    <span className="text-lg text-danger cursor-pointer active:opacity-50">
                      <div
                        onClick={(e) => deletePerguntaFrequente(item.codigo)}
                      >
                        <DeleteIcon />
                      </div>
                    </span>
